...
public final class FileWritingTest {
  ...
  public void pathNestedClasses() throws IOException {
    ...
    JavaFile.builder("new", type).build().writeTo(fsRoot, UTF_8);
    ...
    Path newPath = fsRoot.resolve(fs.getPath("new", "Test.java"));
  ...
  @Test
  public void writeToFileReturnsPath() throws IOException {
    JavaFile javaFile = JavaFile.builder("foo", TypeSpec.classBuilder("Taco").build()).build();
    File filePath = javaFile.writeToFile(Path.of("target/out").toFile());
  ...
  public void coverage() {
    JavaFile sandFile = JavaFile.builder("foo", TypeSpec.classBuilder("Sandwich").build()).build();
    JavaFile tacoFile = JavaFile.builder("foo", TypeSpec.classBuilder("Taco").build()).build();
    assertThat(sandFile).isNotEqualTo(tacoFile);
    assertThat(tacoFile.hashCode()).isNotEqualTo(0);
  }
}
