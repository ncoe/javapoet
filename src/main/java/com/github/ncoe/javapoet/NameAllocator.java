 ...
 * To use, first create an instance and allocate all the names that you need. Typically,
 * this is a mix of user-supplied names and constants: <pre>   {@code
 *
 ...
 * }</pre>
 *
 * <p>Pass a unique tag object to each allocation. The tag scopes the name, and can be used
 * to look up the allocated name later. Typically, the tag is the object that is being named.
 * In the above example we use {@code property} for the user-supplied property names,
 * and {@code "string builder"} for our constant string builder.
 *
 ...
 * }</pre>
 *
 * <p>The above code generates unique names if presented with conflicts. Given user-supplied
 * properties with names {@code ab} and {@code sb} this generates the following:  <pre>   {@code
 *
 *   &#64;Override
 ...
 * }</pre>
 *
 * <p>The underscore is appended to {@code sb} to avoid conflicting with the user-supplied
 * {@code sb} property. Underscores are also prefixed for names that start with a digit,
 * and used to replace name-unsafe characters like space or dash.
 *
 * <p>When dealing with multiple independent inner scopes, use a {@link #clone()} of
 * the NameAllocator used for the outer scope to further refine name allocation for a
 * specific inner scope.
 */
...
public final class NameAllocator implements Cloneable {
   ...
   * by passing {@code tag} to {@link #get(Object)}.
   *
  ...
  /**
   * Return a new name using {@code suggestion} that will
   * not be a Java identifier or clash with other names.
   *
   * @param suggestion the suggestion
   * @return the name
   */
  ...
  /**
   * Make a valid java identifier from {@code suggestion}.
   *
   * @param suggestion the suggestion
  ...
  /**
   * Create a deep copy of this NameAllocator. Useful to create multiple independent
   * refinements of a NameAllocator to be used in the respective definition of multiples,
