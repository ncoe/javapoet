...
/**
 * Any type in Java's type system, plus {@code void}. This class is an identifier for
 * primitive types like {@code int} and raw reference types like {@code String} and {@code List}.
 * It also identifies composite types like {@code char[]} and {@code Set<Long>}.
 *
 * <p>Type names are dumb identifiers only and do not model the values they name. For example,
 * the type name for {@code java.util.List} doesn't know about the {@code size()} method,
 * the fact that lists are collections, or even that it accepts a single type parameter.
 *
 * <p>Instances of this class are immutable value objects
 * that implement {@code equals()} and {@code hashCode()} properly.
 *
 * <h3>Referencing existing types</h3>
 *
 * <p>Primitives and void are constants that you can reference directly:
 * see {@link #INT}, {@link #DOUBLE}, and {@link #VOID}.
 *
 * <p>In an annotation processor you can get a type name instance
 * for a type mirror by calling {@link #get(TypeMirror)}.
 * In reflection code, you can use {@link #get(Type)}.
 *
 * <h3>Defining new types</h3>
 *
 * <p>Create new reference types like {@code com.example.HelloWorld}
 * with {@link ClassName#get(String, String, String...)}. To build composite types
 * like {@code char[]} and {@code Set<Long>}, use the factory methods on {@link ArrayTypeName},
 * {@link ParameterizedTypeName}, {@link TypeVariableName}, and {@link WildcardTypeName}.
 */
public class TypeName {
  ...
  /**
   * This type with additional annotations.
   *
  ...
  /**
   * Copy this type name without annotations.
   *
  ...
  /**
   * Concatenate the current and new annotations in one list.
   *
   * @param annotations the new annotations
   * @return the current and new annotations
   */
   ...
   *
   * @return true if a boxed primitive
   */
  ...
  /**
   * Returns a boxed type if this is a primitive type
   * (like {@code Integer} for {@code int}) or {@code void}.
  ...
  /**
   * Returns an unboxed type if this is a boxed primitive type
   * (like {@code int} for {@code Integer}) or {@code Void}.
  ...
  static TypeName arrayComponent(TypeName type) {
    return type instanceof ArrayTypeName arrayTypeName
      ? arrayTypeName.getComponentType()
      : null;
  }
